...
##--------------------------
setwd('C:/Users/zacha/Documents/GitHub/Transfer_HPC/BCI_vals/')
###***************************
...
rm(var.res.arr) # large file
bci.tower <- read.csv("data-raw/Detto_2022_Dryad_Barro Colorado Island - eddy covariance flux data (2012-2017)/BCI_v5.1.csv", header = TRUE)
...
rm(mod.swc.wide, mod.swc.i, mod.swc.wide.i,var.res.arr, obs.depths, obs.depths.i, non.obs.depths.i)

# For vertical data
load(file.path("data-raw/extract", current.folder, "extract/H2OSOI.h1.extract.Rdata"))
obs.depths <- c(15) #  Vertical probe is from 0-15 cm
## match the indices that those depths correspond to in soil.depths, and thus var.res.arr
obs.depths.i <- c(1:4)
...
mod.swc.wide <- setDT(as.data.frame(t(var.res.arr[[1]])), keep.rownames = "date")
mod.swc.vert <- melt(mod.swc.wide, id.vars = "date", variable.name = "par.sam",
                measure.vars = 2:ncol(mod.swc.wide))
mod.swc.vert[, c("date", "depth"):=list(as.Date(date), obs.depths[1])]

for(i in 2:length(obs.depths)){
  mod.swc.wide.i <- setDT(as.data.frame(t(var.res.arr[[i]])), keep.rownames = "date")
  mod.swc.vert.i <- melt(mod.swc.wide.i, id.vars = "date", variable.name = "par.sam",
                    measure.vars = 2:ncol(mod.swc.wide.i))
  mod.swc.vert.i[, c("date", "depth"):=list(as.Date(date), obs.depths[i])]
  mod.swc.vert <- rbindlist(list(mod.swc.vert, mod.swc.vert.i))
}
rm(mod.swc.wide, mod.swc.vert.i, mod.swc.wide.i,var.res.arr, obs.depths, obs.depths.i, non.obs.depths.i)
...
mod.swc.d <- mod.swc %>% mutate(date.depth = paste0(date, ".", depth)) %>% as.data.frame()
...
write.csv(swc.d.long.vert, file = file.path("results", current.folder, "swc.d.long.vert.csv"), row.names = FALSE)

rm(mod.swc.d, mod.swc.d.1, mod.swc.d.10, mod.swc.d.100, mod.swc.d.3, mod.swc.d.300, mod.swc.d.40, mod.swc.d.5, mod.swc.d.500, mod.swc.d.6, mod.swc.d.800)
rm(mod.sat.d)
rm(g1)
...
swc.d.long.vert <- read.csv(file = file.path("results", current.folder, "swc.d.long.vert.csv"))
