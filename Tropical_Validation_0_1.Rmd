...
Joined$HydraulicCond<-Joined$Variable/10000
```
...
for(i in 1:length(runs)){
  ...
  m1<-lm(Joined_obs$HydraulicCond~Joined_obs$K_obs)
...
ggplot()+
  geom_line(aes(x=Mean_LWP,y=HydraulicCond,fill=Replicate,color=Replicate),dat=Joined,wd=1.0,type="points")+
  geom_line(aes(x=LWP_round, y=K_obs, fill='black'), Observed,lwd=2.0)+
  geom_line(aes(x=Mean_LWP, y=HydraulicCond, fill=alpha('green',0.5)), dat=best_df)+
  theme_classic()
```
